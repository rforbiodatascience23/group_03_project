...
As seen from above, the data is pivoted which will be convenient for calculating the average for each gene.
...
dat_clean |>  
  filter(date == "16-Apr-2009")
```

Alternative handling of replicates without hard coding. On the other hand it takes a lot of time for all the genes..

```{r}
# Just used to refresh the dat_clean
dat_clean <- inner_join(x = count_data,
                        y = meta_data,
                        by = "identifier") |> 
  relocate(colnames(meta_data))

#extract duplicated patinet(s), rename patient_n to 1 identifyer, group by patient and calculate mean across all genes.
dat_rep_patient <- dat_clean |>
  filter(str_detect(patient_n, "_")) |>
  mutate(patient_n = str_extract(string = patient_n, 
                                 pattern = "[0-9]+")) 

dat_rep_mean_gene <- dat_rep_patient |> 
  group_by(patient_n) |>
  #select(1:100) |> #should be removed, but takes a lot of time for all genes
  summarise(across(starts_with("ILMN"), 
                   mean, na.rm = TRUE)) #na.rm to exclude NA from calculations

#extract the metadata from duplicate(s) and keep only one row

collapesed_meta_cols <- dat_rep_patient |>
  group_by(patient_n) |> 
  summarise(geo_accession = paste0(geo_accession, collapse = "_"),
            identifier = paste0(identifier, collapse = "_"))

#extract the metadata from duplicate(s) and keep only one row
rep_meta_cols <- dat_rep_patient |>
  select(patient_n, gender, disease_state, date) |> 
  distinct() |> 
  add_column(collapesed_meta_cols |> select(geo_accession, identifier),
             modified = 1)


#Join mean expression data with extracted metadata from the person
dat_rep_tot <- inner_join(x = dat_rep_meta_cols, 
                          y = dat_rep_mean_gene,
                          by = "patient_n")

dat_clean <- bind_rows(dat_clean, dat_rep_tot) |>
  filter(!str_detect(patient_n, "_")) |>  
  dplyr::select(-patient_n) |> 
  arrange(disease_state)
...
dat_clean |> 
  filter(date == "16-Apr-2009")  # (just used to check that the patient is added)
