...
dat_volcano_plot <- dat_estimates |> 
  mutate(gene_labels = case_when(significant == "Yes" ~ gene, TRUE ~""))
...
volcano_plot <- ggplot(data=dat_volcano_plot, aes(x=estimate,
  ...
  theme_minimal() +
  geom_text_repel(size = 1.5, max.overlaps = 31) +
...
#might be problem that the gene id is not stored correctly
Gene_IDs <- dat_estimates |>
  filter(significant == "Yes") |>
  mutate(gene_id = map( .x = gene,
                   .f = ~ mget(x = .x, 
...
Gene_IDs
#old base R way to extract gene id's
