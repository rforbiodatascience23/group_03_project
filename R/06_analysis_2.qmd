...
In the following a linear regression model is mapped to the data.
In order to do so, the data need to be pivoted to long format and grouped by genes. Gene expression levels and disease label are nested pr. gene.
...
A forest plot is generated showing the genes that are significantly up- or downregulated in tuberculosis patients (TB).

```{r}
#| label: Forrest plot
# - genes are reordered according to estimate
...
A vulcano plot is generated to show ...

```{r}
#| label: Vulcano plot
# add gene_label for plotting
