...
forest_plot <- ggplot(data = dat_forest_plot, 
                      aes(x=estimate, 
                          y=fct_reorder(gene_symbol, estimate), 
                          xmin = conf.low, 
                          xmax = conf.high)) +
  geom_errorbarh() +
  ...
  labs(x = "Estimates (95% CIs)",
       y = "Genes",
       title = "TB-signature",
       subtitle = str_c("Genes Associated with Tuberculosis in HIV patients",
                        "\n",
                        ""),
...
dat_volcano_plot <- dat_estimates |> 
  mutate(gene_labels = case_when(significant == "Yes" ~ gene_symbol, 
                                 TRUE ~ ""))
...
volcano_plot <- ggplot(data = dat_volcano_plot, 
  ...
  labs(x = "Estimates",
       ...
       subtitle = str_c("Genes highlighted in orange were significant after multiple",
                        "\n",
                        "testing correction"),
       caption = "Data from Dawany et al. (2014)") +
...
intersect(significant_genes, signature_251_list)
```
